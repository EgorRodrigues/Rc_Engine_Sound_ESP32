...
volatile int revVolumePercentage = 55; // Adjust the idle volume (usually = 100%, more also working, depending on sound) 55
...
volatile int dieselKnockVolumePercentage = 400; // Adjust the Diesel knock volume (usually = 200 - 600%) 200
...
volatile int turboVolumePercentage = 70; // Adjust the turbo volume (usually = 70%) 40
...
// Clutch parameters ---------------------------------------------------------------------------------------------------
uint16_t clutchEngagingPoint = 10; // CEP. The "clutch" is engaging above this point = engine rpm sound in synch with ESC power (100, 10 for automatic)

// Engine parameters ----------------------------------------------------------------------------------------------------
// Engine max. RPM in % of idle RPM. About 200% for big Diesels, 400% for fast running motors.
uint32_t MAX_RPM_PERCENTAGE = 230; // NOTE! Was called TOP_SPEED_MULTIPLIER (TSM) in earlier versions and was a multiplier instead of a percentage! 270
